...
|                      |        |                                                   |                                                              |          |
| `txSalt`             | string | Random data used to prevent replay attacks        | Non-empty string                                             | Y        |
...
- **FOR EACH INPUT:**
  - **IF** The transaction is a coinbase transaction (From
    ...
    - `idSignature` may not be reused for any issuance (`issuance.signature`)
      or coinbase transaction
    - Issuing sum of all coinbase input amounts must not push the circulating
      supply of the token over it's maximum (`issuance.supply`). If the supply
is unlimited, this is ignored.
  - **ELSE** The transaction is a normal transaction
    - The current balance of `input[n].address` must be sufficient to cover
      `input[n].amount`
- **IF** Any inputs or outputs fail validation, the transaction is ignored
  entirely.
- **IF** The sum of input amounts is greater than output amounts, the unspent
  difference(UTXO) is burnt.



Assuming these criteria are met, each respective `input[n].amount` is deducted
from `input[n].address` and `output[n].amount` is credited to
`output[n].address`. In the case of a coinbase transaction, the deduction does
not occur for `input[n].address` as tokens are being issued into existence.
