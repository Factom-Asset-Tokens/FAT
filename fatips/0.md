...
Token Transaction Entries represent the signed transfer of an amount of FAT-0 tokens from one Factoid address to another.
...
Overall the FAT-0 standard on Factom lends itself to being cross platform. Any client capable of executing HTTP requests to a local or remote Factom full node should be able to implement the standard.
