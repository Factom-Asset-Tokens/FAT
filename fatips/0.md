...
| Name      | Type   | Description                           | Validation                                                   | Required |
| --------- | ------ | ------------------------------------- | ------------------------------------------------------------ | -------- |
| `inputs`  | object | The inputs of this this transaction   | Mapping of Public Factoid Address => Input amount. Amount must be a positive nonzero integer(units of of 10^-10 of a token). Must contain at least 1 Key value pair. No duplicate keys. | Y        |
| `outputs` | object | The outputs of this transaction       | Mapping of Public Factoid Address => Input amount. Amount must be a positive nonzero integer(units of of 10^-10 of a token). Must contain at least 1 Key value pair. No duplicate keys. | Y        |
...
- T.1.2: The JSON must contain all required transaction fields listed in the above table. The fields must comply with all stated validation criteria
- T.1.3: The sum of all values(amounts) in the `inputs` object must be equal to the sum
  of all values(amounts) in the `outputs` object.
- T.1.4: The `inputs` and `outputs` objects may not share common keys (Factoid Addresses)
- T.2.1: The entry hash of the transaction entry must be unique among all transactions belonging to this token
...
- N.1.1: The entry must contain a minimum of `2n` External IDs, where `n` is
  the number of keys in the `inputs` object.
- N.2.1: The Coinbase address
  (`FA1zT4aFpEvcnPqPCigB3fvGu4Q4mTXY22iiuV69DqE1pNhdF2MC`) may not exist as a key in `inputs`.
...
- N.2.3: All signatures in the odd External IDs must be unique across all
...
- C.1.1: The `inputs` object may only contain the public coinbase address as a key (`FA1zT4aFpEvcnPqPCigB3fvGu4Q4mTXY22iiuV69DqE1pNhdF2MC`)
- C.2.1: The coinbase input amount plus the sum of tokens issued in all previous Coinbase transactions must be less than or equal to `supply` from the Issuance entry (if it is provided).
- C.3.1: The last ExtID of the entry must be a valid signature of `token chain id + entry content` by the SK1 key of the token Issuer.
