...
```json
{
    ...
    "chainID": "85ab32f9748bf4c24cedb301bc5a4938eb5cd4a5b0bb46b70027bf983f579a2e",
    //coinbase TX fields
...
| `chainID`            | string | The hex encoded chain ID of the transaction chain | Equal to transaction chain ID                                | Y        |
|                      |        |                                                   |                                                              |          |
...
- T.2.1: The `chainID` field must match the Chain ID of the chain the entry is
  being made on.
...
functionality, there are some recommended implementation guidelines.
...
Sqlite database would suffice. It's important to use disk storage instead of
keeping everything in memory, as a successful token will have an ever
increasing ordered list of transactions and Factoid Addresses with a positive
balance.
