...
An ID string universally unique to Factom identifies each token. This ID is the
only piece of data needed to retrieve the token's data from Factom. It should
be something short and memorable related to your token. For example: `mytoken`
is the token ID of the FAT-0 test token: My Token (MYT).


## Addresses

FAT-0 uses Factom's [Factoid
Address](https://www.factom.com/devs/docs/guide/factom-data-structures) key
pairs based on ed25519 cryptography to send and receive tokens.
...
The issuance Chain ID can be calculated by the Token ID. It is calculated as
such:
...
A datastructure called the Token Issuance holds the information about the FAT-0
token such as it's name, symbol, supply, and ID.

The issuance entry is the content of the first entry on the token's issuance
chain. There is only one issuance entry per FAT-0 token (the first entry).
...
A Factom chain is used to hold the transactions of the token's issuance. This
allows time series locking of transactions, a requirement for validation.


### Transaction Chain ID `extends` [FATIP-100](100.md)

The transaction Chain ID can be calculated by the Token ID. It is calculated as
such:
