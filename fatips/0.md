...
|                      |        |                                                   |                                                              |          |
| `declaredHeight`     | number | A block height for which the transaction will be considered valid, and within which the signatures in the External IDs must be unique | Less than or equal to the block height containing the entry, and greater than one less the block height containing the entry. | Y        |
...
| 1                 | Signature of entry content by key of RCD in preceding External ID |
| ...               |                                                                   |
| `X`               | RCD corresponding to `input[X/2].address`                         |
| `X + 1`           | Signature of entry content by key of RCD in preceding External ID |
|                   |                                                                   |
...
- T.2.2: All `address` fields in the `inputs` and `outputs` arrays must contain a
  valid human readable public Factoid Address.
- T.2.3: No Factoid Address may be used in more than one input or output. e.g.
...
- T.2.4: All `amount` fields in the `inputs` and `outputs` arrays must be a
  positive (non-zero) integer.
- T.2.5: The sum of the `amount` field in the `inputs` must be equal to the sum
...
- N.2.1: The `declaredHeight` must be equal or one less than the block height
  of the transaction entry. In other words, if `height` is the actual block
height of the entry, then `declaredHeight` must be in the range `[height - 1,
height]`, inclusive.
...
uniqueness only needs to be checked among valid transactions with the same
`declaredHeight`.
...
- N.3.2: If `i` is odd, then the `i`th External ID must be the raw data of a
  valid signature of the content of the entry, verified against the public key
stored in the RCD in the `i-1`th External ID.
...
- C.2.3: The `idSignature` must be unique across all previously valid Coinbase
  transactions.
- C.3.1: The `idSignature` must be a valid signature of the `salt` transactions
  by the SK1 key specified by the Issuer's Identity Chain.
